    ...
    public void generateEntityIcons()
    {
        ...
        foreach (Transform e in Entities)
        {
            ...
            if (icon == null)
            {
                ...
                {
                    --count;
                    icon = (Texture2D)UnityEditor.AssetDatabase.GetCachedIcon(UnityEditor.AssetDatabase.GetAssetPath(UnityEditor.PrefabUtility.GetCorrespondingObjectFromSource(e.gameObject)));
